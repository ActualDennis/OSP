...
void print_info(int num, int get){
    char got_sent_info[50];
    if (get == 0){
        strcpy(got_sent_info,"got ");
    }else
        strcpy(got_sent_info, "sent "); 

    strcat(got_sent_info, "USR1");

    printf("%d %d %d %s %d\n", num, getpid(),getppid(), got_sent_info, get_current_time());
}

void print_term_info(int usr1){
    printf("%d %d ended work after %d SIGUSR1 \n", getpid(), getppid(),usr1);
}
